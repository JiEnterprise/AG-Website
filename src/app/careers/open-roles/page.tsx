...
const DEPT_COLORS: Record<string, string> = {
  ...
  'Wealth Management': 'text-aurum-gold border-[rgba(201,168,76,0.3)]',
  ...
  'Internship': 'text-[#C8B8A8] border-[rgba(200,184,168,0.3)]',
    ...
    <main className="bg-obsidian text-pale-gold min-h-screen">
      ...
      <section className="relative min-h-[60vh] flex flex-col justify-end overflow-hidden border-b border-[rgba(201,168,76,0.1)]">
        ...
        <div className="relative z-10 max-w-[1240px] mx-auto w-full px-6 sm:px-10 pb-20">
          ...
          <p className="font-dm text-[16px] leading-relaxed text-[#C8BEA8] max-w-[520px]">
      ...
      <section className="max-w-[1240px] mx-auto px-6 sm:px-10 py-20">
        <div className="flex flex-col gap-4">
          {ROLES.map((role, i) => (
            <div key={i} className="group rounded-2xl border border-[rgba(201,168,76,0.1)] bg-[rgba(17,17,20,0.6)] p-7 hover:border-[rgba(201,168,76,0.28)] hover:-translate-y-0.5 transition-all duration-200 flex flex-col sm:flex-row sm:items-center gap-5">
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2 mb-3">
                  <span className={`font-dm text-[9px] uppercase tracking-[0.14em] px-2.5 py-1 rounded-full border ${DEPT_COLORS[role.department] || 'text-aurum-gold border-[rgba(201,168,76,0.3)]'}`}>
                    {role.department}
                  </span>
                  <span className="font-dm text-[9px] uppercase tracking-[0.14em] px-2.5 py-1 rounded-full border text-[#8A8070] border-[rgba(138,128,112,0.3)]">
                    {role.type}
                  </span>
                </div>
                ...
                <p className="font-dm text-[12px] text-[#5A5040] mb-3">{role.location}</p>
                <p className="font-dm text-[12px] text-[#8A8070] leading-relaxed max-w-[640px]">{role.desc}</p>
              </div>
              <a href="/sign-in" className="flex-shrink-0 inline-flex items-center gap-2 px-6 h-10 border border-[rgba(201,168,76,0.3)] font-dm text-[11px] uppercase tracking-[0.1em] text-aurum-gold hover:bg-aurum-gold hover:text-[#0A0800] transition-all duration-200 rounded-full whitespace-nowrap">
      ...
      {/* CTA */}
      <section className="border-t border-[rgba(201,168,76,0.1)] max-w-[1240px] mx-auto px-6 sm:px-10 py-24 text-center">
        ...
        <p className="font-dm text-[14px] text-[#8A8070] leading-relaxed max-w-[400px] mx-auto mb-8">
