  ...
  return (
    <main className="bg-obsidian text-pale-gold min-h-screen">
      ...
      {CATEGORIES.map((cat, i) => (
        <section key={cat.title} className={`max-w-[1240px] mx-auto px-6 sm:px-10 py-20 ${i < CATEGORIES.length - 1 ? 'border-b border-[rgba(255,255,255,0.09)]' : ''}`}>
          <p className="font-dm text-[10px] uppercase tracking-[0.28em] text-aurum-gold mb-10">{cat.title}</p>
          <div className="grid sm:grid-cols-2 gap-4">
            {cat.items.map(item => (
              <div key={item.name} className="rounded-2xl border border-[rgba(255,255,255,0.09)] bg-[rgba(255,255,255,0.04)] p-7 hover:border-[rgba(255,255,255,0.18)] transition-all duration-200">
                <h3 className="font-playfair text-[18px] text-pale-gold mb-2">{item.name}</h3>
                <p className="font-dm text-[13px] text-[#86868B] leading-relaxed">{item.detail}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      {/* CTA */}
      <section className="border-t border-[rgba(255,255,255,0.09)] max-w-[1240px] mx-auto px-6 sm:px-10 py-28 text-center">
