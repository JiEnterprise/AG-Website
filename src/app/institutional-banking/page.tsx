    ...
    <main className="bg-obsidian text-pale-gold min-h-screen">
        ...
        <div className="grid sm:grid-cols-3 gap-12">
          {STATS.map(s => (
            <div key={s.label}>
              ...
              <p className="font-dm text-[13px] text-[#8A8070] uppercase tracking-[0.16em]">{s.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-[1240px] mx-auto px-6 sm:px-10 py-24 border-b border-[rgba(201,168,76,0.1)]">
        ...
        <div className="grid sm:grid-cols-3 gap-6">
          {FEATURES.map(f => (
            <div key={f.title} className="rounded-2xl border border-[rgba(201,168,76,0.1)] bg-[rgba(17,17,20,0.6)] p-8 flex flex-col group hover:border-[rgba(201,168,76,0.28)] hover:-translate-y-1 transition-all duration-200">
              ...
              <p className="font-dm text-[13px] text-[#8A8070] leading-relaxed flex-1">{f.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-[1240px] mx-auto px-6 sm:px-10 py-24 border-b border-[rgba(201,168,76,0.1)]">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-0">
          {OFFERINGS.map(item => (
            <a key={item} href="#" className="group flex items-center justify-between py-5 border-b border-[rgba(201,168,76,0.07)] hover:border-[rgba(201,168,76,0.2)] transition-colors duration-150 pr-4">
              <span className="font-playfair text-[20px] text-[#C8BEA8] group-hover:text-pale-gold transition-colors duration-150">{item}</span>
              <ChevronRight size={15} strokeWidth={1.5} className="text-[#3A3328] group-hover:text-aurum-gold transition-colors duration-150 flex-shrink-0" />
            </a>
          ))}
        </div>
      </section>
